...
export default function EditTrip() {
  ...
  useEffect(() => {
    ...
    const fetchTrip = async () => {
      try {
        ...
        if (data.user_id !== user.id) throw new Error('No tienes permiso para editar este viaje');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!user) throw new Error('Debes iniciar sesión para editar un viaje');
  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] bg-gray-50 py-12 dark:bg-primary">
      <div className="mx-auto w-full max-w-2xl px-4 sm:px-6 lg:px-8">
        ...
        >
          <div>
            <h1 className="text-3xl font-bold text-primary dark:text-white">Editar Viaje</h1>
            <p className="mt-2 text-primary-light dark:text-gray-300">Modifica los detalles de tu viaje</p>
          </div>
